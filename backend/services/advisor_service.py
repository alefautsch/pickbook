...
def build_inseason_context(db: Session, league_id: str) -> dict[str, Any]:
    ...
    team_detail = get_team_detail(db, league_id, my_roster.sleeper_roster_id)
    ...
    return {
        ...
        "my_team": {
            ...
            "starter_ppg_rank": team_detail.starter_ppg_rank,
            "tv_rank": team_detail.tv_rank,
            "avg_dynasty_rating": team_detail.avg_dynasty_rating,
            "starter_total_ppg": team_detail.starter_total_ppg,
            "total_trade_value": team_detail.total_trade_value,
            "contender_tier": team_detail.contender_tier,
            "contender_score": team_detail.contender_score,
            "traits": [t.model_dump() for t in team_detail.traits],
        },
        "my_roster": {
            "starters": [
                {
...
def stream_advisor_chat(
    ...
    *,
    ...
    if not settings.anthropic_api_key:
        raise ValueError(
            "ANTHROPIC_API_KEY is not configured. Add it to .env to use the advisor."
        )

    row = advisor_model_by_id(model_id)
    if row["provider"] != "anthropic":
        raise ValueError("Blackbook advisor currently supports Anthropic models only.")
    ...
    context = build_inseason_context(db, league_id)
